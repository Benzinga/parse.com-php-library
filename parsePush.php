...
class parsePush extends parseRestClient{
  ...
  public function __set($name,$value){
    if($name != 'push_time' || $name != 'channel' || $name != 'channels' || $name != 'expiration_time' || $name != 'expiration_interval' || $name != 'type' || $name != 'data' || $name != 'where'){
    ...
    if($this->_globalMsg != ''){
      ...
      if(count($this->data) > 0){
        ...
        if(!empty($this->expiration_time)){
          $params['expiration_time'] = $this->expiration_time;
        }
        ...
        if(!empty($this->push_time)) {
          $params['data']['push_time'] = $this->push_time;
        }
